...
// 外部舵机控制器声明（在wings_control_ux_enhanced.c中定义）
...
static esp_err_t sequence_steps_handler(httpd_req_t *req) {
    ...
    for (int i = 0; i < frame_count && i < MAX_SEQUENCE_STEPS; i++) {
        ...
        if (seq_mgr_get_frame(&g_sequence_mgr, i, &frame) == ESP_OK) {
            ...
            int delay_ms = 0;
            if (i > 0) {
                action_frame_t prev_frame;
                if (seq_mgr_get_frame(&g_sequence_mgr, i - 1, &prev_frame) == ESP_OK) {
                    delay_ms = (int)(frame.timestamp - prev_frame.timestamp);
                }
            }
            snprintf(step_str, sizeof(step_str), 
                     "{\"index\":%d,\"left_angle\":%d,\"right_angle\":%d,\"timestamp\":%lu,\"delay_ms\":%d}",
                     i, frame.left_angle, frame.right_angle, frame.timestamp, delay_ms);
...
// 播放序列处理（需要集成到舵机控制）
// 播放回调函数 - 用于设置舵机角度
static esp_err_t play_servo_callback(int servo_id, int angle, int speed) {
    return servo_set_angle_and_speed(&g_servo_ctrl, servo_id, angle, speed);
}
...
static esp_err_t sequence_play_handler(httpd_req_t *req) {
    ...
    if (frame_count <= 0) {
        snprintf(response, sizeof(response), 
                 "{\"success\":false,\"message\":\"没有可播放的序列数据\"}");
    } else {
        // 执行序列播放
        esp_err_t err = seq_mgr_play_sequence(&g_sequence_mgr, play_servo_callback);
        if (err == ESP_OK) {
            snprintf(response, sizeof(response), 
                     "{\"success\":true,\"message\":\"序列播放完成\",\"frame_count\":%d}", 
                     frame_count);
        } else {
            snprintf(response, sizeof(response), 
                     "{\"success\":false,\"message\":\"播放失败: %s\"}", 
                     esp_err_to_name(err));
        }
    }
...
void register_sequence_handlers(httpd_handle_t server) {
    ...
    // 停止播放
    httpd_uri_t stop_play_uri = {
        .uri = "/sequence/stop_play",
        .method = HTTP_GET,
        .handler = sequence_stop_play_handler,
        .user_ctx = NULL
    };
    httpd_register_uri_handler(server, &stop_play_uri);
